    ...
    if (topic && message && client) {
      client.publish(topic, message, { qos: 1, retain: false }, (err) => {
        if (err) {
          ...
          socket.emit('mqtt_success', { topic, message });
          // Broadcast to all clients
          io.emit('mqtt_published', { topic, message, timestamp: new Date() });
        }
      });
    } else {
      console.warn('⚠️ Invalid MQTT publish request:', data);
      socket.emit('mqtt_error', { error: 'Missing topic or message' });
    }
  });
